...
Message:
${message}`,
					html: `<p><b>From:</b> ${name} &lt;${email}&gt;</p>${phone ? `<p><b>Phone:</b> ${phone}</p>` : ''}${linkedin ? `<p><b>LinkedIn:</b> ${linkedin}</p>` : ''}<p>${message.replace(/\n/g, '<br>')}</p>`
				});
				emailOk = true;
			} catch (err) {
		...
		if (!attemptedAny) {
			return res.status(500).json({ ok: false, error: 'No delivery channel configured. Set SMTP_* or GOOGLE_* env variables.' });
		}
		if (!emailOk && !formOk) {
...
// Serve presentations (local files or redirect to Google Slides)
// Option A: redirect to a single Google Slides deck via env (or default)
// PRESENTATION_URL can be a Google Slides share link
...
app.get('/presentations', (req, res) => {
    ...
    // Option B: serve a local file if provided (e.g., /assets/slides.pdf)
